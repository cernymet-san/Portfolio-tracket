...
#jaký produkt měřím +
excel_name = 'B test_DES_data_DES_CEPS CERT_CEPS_mFRR125_B_3MW.xlsx'
...
color_in_hex = sh['A%s'%(ramp_second_command_t2)].fill.start_color.index #select cell where ramp_second... is a number
...
value_found=0
for x in range(excel_source.shape[0]-1):
    if math.isnan(P_skut[x])==False and math.isnan(P_skut[x+1])==False: # Check if current and next P_skut values are not NaN
        if round(P_skut[x],2)==(round(P_dg+P_cil,2)) and value_found==0 and test_time[x] < ramp_length+60: #changed == for <= and added abs for Pskut # Check if P_skut reaches target
...
if P_dg==0: #test A
    ...
    for x in range(excel_source.shape[0]-1):
        if math.isnan(P_skut[x])==False and math.isnan(P_skut[x+1])==False:
            if test_time[x]>ramp_second_command_t2 and round(P_skut[x],2)<=P_dg and value_found==0 : # Check if test_time equals ramp_second_command_t2 - 60 + ramp_length ?? Proč?
                reached_P_dg_t3 = x+1
                value_found=value_found+1  # Indicate that the target has been reached
...
if P_dg==0: #mFRR+ and TEST A
    ...
    data_t1_t2 = P_skut[reached_P_cil_t1:ramp_second_command_t2] #Select the data between t1 (reached_P_cil_t1) and t2 (ramp_second_command_t2)
    mean_data_t1_t2 = stc.mean(P_skut[reached_P_cil_t1:ramp_second_command_t2])     # Calculate the mean of the selected data
...
if P_dg==0: #mFRR+ and TEST A
    ...
    plt.annotate(text='', xy=(reached_P_cil_t1,min(P_lim_plus)-0.6), xytext=(beginning_t0,min(P_lim_plus)-0.6), arrowprops=dict(arrowstyle='<->')) #line between t0 and t1
    plt.text((beginning_t0+(t_akt/3))+5, min(P_lim_plus)-0.5, (r'$ t_{AKTmFRR12,5} = %s (s) $'%(t_akt)))
    plt.annotate(text='', xy=(reached_P_dg_t3,min(P_lim_plus)-0.6), xytext=(ramp_second_command_t2,min(P_lim_plus)-0.6), arrowprops=dict(arrowstyle='<->')) #line between t2 and t3
    plt.text((ramp_second_command_t2+(t_deakt/3))+5, min(P_lim_plus)-0.5, (r'$ t_{DEAKTmFRR12,5} = %s (s) $'%(t_deakt)))

    plt.title(r'$ Průběh\ testu\ mFRR_{12.5B}\ -\ grafy:\ P_{cíl}\ =\ P_{DG}\ ±\ mFRR_{ZAD}\ =\ f(t);\ PSKUT\ =\ f(t) $', **font)
    plt.xlabel(r'$ Čas\ [s]$', **font2)
    plt.ylabel(r'$Výkon\ [MW]$', **font2)
