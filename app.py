...
with st.sidebar.expander("📥 Import Portfolio (.xlsx)", expanded=False):
    portfolio_xlsx_file = st.file_uploader("Upload portfolio XLSX", type=["xlsx"], key="portfolio_xlsx_upload")
    overwrite_xlsx = st.checkbox("Overwrite existing portfolio", value=False, key="portfolio_xlsx_overwrite")
    st.caption("Expected columns: Symbol, Shares, Purchase Price")
    if portfolio_xlsx_file and st.button("Import Portfolio XLSX", use_container_width=True):
        with tempfile.NamedTemporaryFile(delete=False, suffix=".xlsx") as tmp:
            tmp.write(portfolio_xlsx_file.getbuffer())
            tmp_path = tmp.name
        try:
            p.import_portfolio_xlsx(tmp_path, overwrite=overwrite_xlsx)
            fetch_last_close.clear()
            fetch_history_value.clear()
            st.success("Imported portfolio from XLSX")
            st.rerun()
        finally:
            try:
                os.remove(tmp_path)
